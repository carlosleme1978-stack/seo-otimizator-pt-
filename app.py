...
gsc_property = st.sidebar.text_input(
    "URL do site no Search Console",
    help="Exemplo: https://teusite.pt/ (tem de estar verificado no GSC).",
)
...
def get_gsc_service():
    if "gsc_creds" in st.session_state:
        creds = st.session_state["gsc_creds"]
    else:
        flow = InstalledAppFlow.from_client_secrets_file(
            "client_secret.json", SCOPES
        )
        creds = flow.run_local_server(port=0)
        st.session_state["gsc_creds"] = creds
